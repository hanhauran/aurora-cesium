...
public final class CzmlGenerator {
    ...
    private void writeEntity(CesiumOutputStream cos, CesiumStreamWriter csw, Entity entity) {
        Optional.ofNullable(entity).ifPresent(et -> et.dispatchEntity(csw.openPacket(cos)));
    }

    private void writeStartElement(CesiumOutputStream cos, CesiumStreamWriter csw, StartElement startElement) {
        Optional.ofNullable(startElement).ifPresent(se -> se.dispatchStartElement(csw.openPacket(cos)));
    }
}
